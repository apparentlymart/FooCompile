...
public class Backend_CLR implements Backend {
    HashMap uniqtable;
    HashSet imports;
    int nextuniq;
    Module m;
    ...
    public Backend_CLR(Module m, PrintStream pw) {
    ...
    public String typeAsIL(Type t) {
        if (t.arraymod > 0) {
            return "array";
        }
        StringBuffer sb = new StringBuffer();
        if (t.isIntrinsic) {
            sb.append(t.intrinsicType.name);
            if (t.size > 0) {
                sb.append(t.size);
            }
            else {
                if (t.intrinsicType.name.equals("int") || t.intrinsicType.name.equals("float")) {
                    sb.append("32");
                }
            }
        }
        else {
            // FIXME: Need to get the entire type, including any .NET assembly reference
    ...
    public String functionAsIL(Invokable inv) {
        StringBuffer sb = new StringBuffer();

        sb.append(typeAsIL(inv.returnType));
        sb.append(" ");
        sb.append(inv.name);
        sb.append("(");
        Iterator iNFD = inv.params.iterator();
        int iNFDcounter = 0;
        while (iNFD.hasNext()) {
            Variable p = (Variable)iNFD.next();
            String param = p.name;
            if (iNFDcounter > 0) sb.append(", ");
            sb.append(typeAsIL(p.type)+" "+param);
            iNFDcounter++;
        }
        sb.append(")");
    ...
    public void printTopLevelFunctions() {
        Invokable inv;
        ...
        while (i.hasNext()) {
            ...
            else throw new InternalErrorException("Global invokables can only be functions.");

            out.print(".method public static "+functionAsIL(inv)+" il managed");

            out.println("{");
            if (inv.name.equals("main")) {
    ...
    public void evaluateExpression(NodeExpr expr, Scope scope) {
        ...
        //Function Call
            ...
            out.println("call "+functionAsIL(inv));
        }
        //Boolean Literal
    ...
    public class ASMWriter {
        ...
        public void printHeader(Module m) {
            pw.println("// Generated by FooCompile, CLR backend");
            pw.println(".assembly fooprog {}");
        }
    ...
    public static class BackendException extends RuntimeException {
        ...
        public static BackendException unsupportedFeature(String feature) {
            return new BackendException("The CLR backend does not support the use of " + feature);
        }
