...
public class NodeOrOp extends NodeOperation {
    NodeExpr lhs;
    NodeExpr rhs;
    boolean bitwise;

    public NodeOrOp(NodeExpr lhs, NodeExpr rhs, boolean bitwise) {
        this.lhs = lhs;
        this.rhs = rhs;
    ...
    public static boolean canStart(Tokeniser t) {
        return NodeAndOp.canStart(t);
    }

    public static Node parse(Tokeniser t) {
        int startline = t.getLine(), startcolumn = t.getColumn();
        NodeExpr lhs = (NodeExpr)(NodeAndOp.parse(t));
        boolean bitwise = false;
        Token tok = t.peek();
        if (tok == TokenSymbol.LOGOR || tok == TokenSymbol.BITOR) {
            bitwise = (tok == TokenSymbol.BITOR);
            t.read();
            return setPos(new NodeOrOp(lhs, (NodeExpr)(NodeExpr.parse(t)), bitwise), startline, startcolumn);
        }
    ...
    public String toString() {
        return "NodeOrOp(" + lhs + ( bitwise ? "|" : "||" ) + rhs + ")";
    }
